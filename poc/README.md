...
│
├── run.sh                  Full validation: fmt → clippy → build → test → run → bench
...
- Benchmark kernels in `benches/` (run `benches/run.sh`): the measured emulation overhead on existing hardware informs the practical plane; it never shapes the model.
