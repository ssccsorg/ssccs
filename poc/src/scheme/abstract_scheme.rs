// Qs core/poc/src/scheme/abstract scheme.rs
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            StructuralRelation::Dependency {
                ...
                .field("dependency_type", dependency_type)
                .field("strength", strength)
                .finish(),
            StructuralRelation::Equivalence {
                equivalence_class,
                symmetry,
            } => f
                .debug_struct("Equivalence")
                .field("equivalence_class", equivalence_class)
                .field("symmetry", symmetry)
                .finish(),
            StructuralRelation::Custom { name, predicate: _ } => f
                .debug_struct("Custom")
                .field("name", name)
...
impl std::fmt::Debug for StructuralConstraint {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("StructuralConstraint")
...
impl std::fmt::Debug for ConstraintScope {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ConstraintScope::Global => write!(f, "Global"),
            ConstraintScope::Local(seg) => write!(f, "Local({:?})", seg),
            ConstraintScope::Regional(segs) => write!(f, "Regional({:?})", segs),
            ConstraintScope::Dimensional(dim) => write!(f, "Dimensional({})", dim),
            ConstraintScope::Relational(rel) => write!(f, "Relational({:?})", rel),
        }
    }
}
...
impl std::fmt::Debug for RelationGraph {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RelationGraph")
            ...
            .field("incoming", &self.incoming.keys().collect::<Vec<_>>())
            .finish()
    }
}
