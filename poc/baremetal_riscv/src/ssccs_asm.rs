...
//!
//! Each concept (observe, collapse, etc.) has its own `.S` file under `asm/`.
//! The `asm_data` module (generated by `build.rs`) exposes `.8byte` constants
//! from each file as a submodule.
//!
//! Validation strategy:
...
// ============================================================================
// Per-concept assembly imports
// ============================================================================
// Each concept has: global_asm! include, extern "C" declarations, and fallback.

// ── Concept: observe_full ───────────────────────────────────────────────
...
mod tests {
    ...
    // Read golden anchors DIRECTLY from each assembly file.
    // If a .S changes, its anchors must change, or the test fails.
    ...
    #[test]
    fn test_observe_full_golden_anchors_present() {
        ...
        let broad = parse_golden(ASM_OBSERVE_FULL, "GOLDEN_BROAD");
    ...
    fn test_composition_vs_golden() {
        let seg = parse_golden(ASM_OBSERVE_FULL, "GOLDEN_SEGMENTS");
        let expected_narrow = parse_golden(ASM_OBSERVE_FULL, "GOLDEN_NARROW");
        let expected_broad = parse_golden(ASM_OBSERVE_FULL, "GOLDEN_BROAD");
