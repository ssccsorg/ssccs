...
subtitle: "Development plan and measured progress: profile gate outcome, physical layout decision, and the lattice extension scaffold for multi-dimensional point and range queries in the DuckDB fork"
date: 2026-08
...
This report is the development plan and the measured progress for a coordinate lattice access path in the DuckDB fork. The problem is scoped to DuckDB: multi-dimensional point and range queries over bounded integer dimensions run as a full table scan for every multi-filter query, and as an ART index scan only below a fixed selectivity threshold for single-filter queries. The plan is benchmark-driven. Phase 1 ran a profile gate with the installed DuckDB package and no C++ build, and the gate passes: the scan is the entire cost of the multi-filter point query. Phase 2 is in progress: the physical layout decision is lattice-sorted storage, and the lattice extension scaffold builds and registers on the fork branch `54-duckdb-lattice`. Phase 3 will record either a measured win through the extension system or a measured negative result. The syntagma side is scoped as a minimal additive core upgrade, implemented only if the prototype supports it. The CERN ROOT-Coord, ExaVerif CVA6, and LLM tracks are reference experiences that set the expectation; every claim in this report is measured on DuckDB.
...
The first task, the physical layout decision, is complete. Lattice-sorted storage: a one-time conversion rewrites the table in lattice order, a cell maps to a row offset in closed form, and the dimension columns become positional. The gate data decided the choice: the cell-to-row map alternative would add about 80 MB to the 92 MB file and must be maintained on append, while lattice order removes the dimension storage and the scan together.

The extension scaffold (`lattice_scan` table function) builds and registers on the fork branch `54-duckdb-lattice`; the addressing implementation, which resolves the table schema, dimension extents, and the query box, and emits the matched rows in closed form, is the next task.
...
The upgrade spec is drafted in parallel with the gate. The implementation lands in Phase 2 only if the prototype supports it, and the tests live in the `/tests` folder.
...
Phase 1 is complete and the gate passes. Phase 2 is in progress on the fork branch `54-duckdb-lattice` in `ssccsorg/duckdb`: the physical layout decision (lattice-sorted storage) is recorded under `benchmark/lattice/layout_decision.md`, the pre-integration data under `benchmark/lattice/results`, and the extension scaffold under `extension/lattice`. The immediate next step is the addressing implementation in the extension, which engages the syntagma core upgrade scope. The track issue is #54 in `ssccsorg/syntagma`.
