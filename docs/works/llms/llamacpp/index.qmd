...
The benchmark results and insights above are recorded from the Phase A and Phase B measurements on the CPU-only build. The investigation continues. The Phase A profile is complete: the in-graph scatter share of decode is 0.025% measured and 0.45% against the baseline step. The remaining items are the SWA variant on a sliding-window model and the GPU-side behavior, which stays on the vLLM CUDA track. The hash-IO replacement value of the tagma map work is scoped as a separate investigation, because the llama.cpp KV path does not carry hash-indexed lookup.
