...
- **Diagnosis Report** (`/docs/report/diagnosis.md`) – offers a rigorous gap assessment and a three‑track implementation roadmap (Field Synthesis, Hardware Mapping, Compiler Optimisations).
...
5. **Benchmarking Suite** (new crate `benchmarks`):
   ...
   - Measure execution time and (where possible) energy using hardware performance counters.
...
3. **Observation‑Code Generation – FPGA** (`hardware‑mapping` crate):
   ...
   - Synthesise the Verilog for a low‑cost FPGA using Yosys/nextpnr.
   - Measure power consumption with on‑board monitors and compare against a soft‑core RISC‑V running the same kernel.
...
5. **Formal Verification** (separate Lean/Coq project):
   ...
   - Integrate the proofs into the CI pipeline.
...
4. **Tooling and Ecosystem**:
   ...
   - Develop a graphical visualiser for Schemes and Fields.
