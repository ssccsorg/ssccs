...
Internally, ExaVerif treats the YAML description as a **Spec Space**: field domains define its axes, constraints carve admissible subspaces, and each combination is a point within this space. The Field Composition engine — a branchless, constant‑time verification kernel validated via x86‑64 CI — guarantees that every evaluation is deterministic. The same input always produces the same result, independent of random seeds or simulator state.
...
ExaVerif is an independent domain entity built on the shared substrate — a working demonstration of the model described in the [SSCCS Project Model](https://docs.ssccs.org/projects). Its Spec Space exploration follows the same Fact / Intent / Hint cycle that governs every Nexus entity internally: domain expansion is an Intent, each evaluated combination a Fact, each constraint a Hint. ExaVerif is a Nexus instance itself, self-similar across scales. Every verification run feeds the shared knowledge store.
