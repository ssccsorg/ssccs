...
Code-level analysis of all types -- including `Coord` bit layout, `CoordSpace` inline array with niche optimization, `CoordSet` bit iteration with `trailing_zeros`, and `CoordSpaceN` sparse tree with lazy node allocation -- is in the reference implementation report [Reference Implementation](./impl.html).
