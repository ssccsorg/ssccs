---
title: "Tagma"
subtitle: "Hashless spatial primitive on a fixed 16-bit Unicode coordinate space"
date: 2026-07
...
## CoordCube: D-dimensional spatial interpretation {#sec-coordcube}

CoordPath treats N syllables as a linear sequence: each syllable is an independent axis with 11,172 values, giving 11,172^N total addressable space. The same bytes can be reinterpreted as a D-dimensional grid via `CoordCube<N, D, R>` where N = D × R. Each dimension now has R syllables of resolution, sacrificing per-syllable precision for spatial semantics — a dimension can represent region, timestamp, or any application-defined quantity, and operations on that dimension (proximity, range, distance) become arithmetic on the bytes.

CoordCube does **not** replace or modify the storage tree. It is a **path generator** that feeds the existing `CoordSpaceN` or `DynCoordSpace` tree — the same tree that handles single-path `get`/`insert`/`remove`. The tree remains the store; CoordCube adds a new way to ask which paths to look up. Memory overhead from CoordCube is zero: all spatial operations are register-only arithmetic on the existing byte array.

### Precision tradeoff

| Criterion | CoordPath (1D) | CoordCube (D-dim) |
|-----------|---------------|-------------------|
| Independent axes | N (every syllable) | D (groups of R syllables) |
| Precision per axis | 11,172 (one syllable) | 11,172^R (R syllables bundled) |
| Granularity | 1 syllable (fine, per-axis) | R syllables (coarse, bundled) |
| Query capability | Exact lookup, slice by axis value | Proximity, bounding box, distance metrics |
| Performance driver | Per-access latency (0.39 ns) | Adjacency throughput (96-699 Melem/s) |
| Use case | Key-value addressing | Spatial region queries |
| Conversion cost | Baseline | 960 ps from CoordPath (no data copy) |

A 6-syllable path (N=6) can be interpreted as CoordPath<6> (6 independent axes, 11,172 precision each) or CoordCube<3, 2> (3 spatial dimensions, 11,172^2 ≈ 1.25 × 10^8 precision each). The same bits serve both views; the user chooses based on workload.

**Granularity tradeoff.** CoordPath adds one independent axis per syllable —
fine-grained control over the address space at a cost of O(N) tree traversal
per access. CoordCube groups R syllables into a single dimension, reducing
the number of independent axes from N to D but gaining spatial semantics:
proximity, bounding box, and distance metrics become arithmetic operations.
The coarse step (R syllables per dimension) is compensated by adjacency-driven
throughput — generating all 5^2 = 25 paths in a 2D neighborhood at radius 2
costs 137 ns (182 Melem/s). Cube advantages are spatial
in sparse stores, Tree+Cube proximity r=1 achieves 3.3x speedup vs sequential Paths (48 ns vs 159 ns). The user chooses: fine-grained linear
control (CoordPath) or coarse-grained spatial power (CoordCube).

### Cost comparison: CoordPath lookup vs CoordCube region generation

CoordPath direct array lookup costs a constant 0.39 ns regardless of data volume — a single load from a 22 KB inline array. CoordCube proximity generation does not answer "what value is at this exact key?" but "all keys within distance r of this center," a query with no equivalent in hash-based systems.

| Combination | Operation | Cost | Scaling |
|------------|-----------|------|---------|
| Dense + CoordPath (native) | Single element access | 0.39 ns | O(1) flat |
| Tree + CoordPath (fallback) | Single element access | 2.69-58.6 ns | O(N) linear |
| Tree + CoordCube (spatial) | Proximity r=1: path gen | 93 ns | 96 Melem/s (path gen) |
| Tree + CoordCube (spatial) | Proximity r=1: full KV (sparse) | 48 ns | 3.3x vs sequential |
| Tree + CoordCube (spatial) | Proximity r=1: full KV (dense) | 287 ns | 9 lookups + Vec push |
| Tree + CoordCube (spatial) | Proximity r=5 (121 paths) | 343 ns | 353 Melem/s |
| Tree + CoordCube (spatial) | Bounding box 10K paths | 14.6 µs | 699 Melem/s |
| Any + CoordCube | Distance metrics | ~320 ps | Register-only |

Distance metrics cost approximately 320 ps regardless of channel — comparable to or below a single CoordSpace array lookup (390 ps) — because all computation is register-only with no memory access. The compiler inlines and unrolls hamming, euclidean, and manhattan to the point where measurement noise dominates.

Proximity generation throughput increases monotonically with radius as the fixed setup cost (~8.6 ns) is amortized: at radius 1 (9 paths) 96 Melem/s; at radius 5 (121 paths) 353 Melem/s. Dimensional scaling follows output path count (2r+1)^D, not N directly.

### Storage-backed queries

The CoordCubeKV trait combines region generation (CoordCube) with tree lookup (CoordSpaceN). Total query time breaks down as:

- Path generation: ~15 ns (5% of total, for D=2, R=1, r=1)
- Tree traversal: ~196 ns (69%, 9 CoordSpaceN lookups at 21.8 ns each)
- Vec push: ~74 ns (26%, allocating result storage)

Memory overhead from CoordCube is zero: all three phases use only registers and the existing tree. The tree allocation strategy (per-prefix, not per-entry) is unchanged — CoordCube neither allocates nor preallocates.
Sparse stores that fit in L1 cache reduce total time to 47.8 ns (188 Melem/s); empty stores measure 15.4 ns — pure generation cost.

See Fig. [fig-bench-spatial] for the combined throughput comparison across both channels.

The architectural significance: no hash-based system can replicate this dual channel. A HashMap key — a uniformly distributed digest — cannot be reinterpreted as a spatial coordinate; the hash function destroys proximity, axis structure, and geometry. A CoordPath can, because its structure is the address. The conversion is 960 ps and changes no state.
...
**Graph adjacency and multi-dimensional query.** Graph engines check adjacency via hash lookups or index intersections ($O(\deg(v))$) and query multi-dimensional attributes via composite indexes or join operations. Tagma represents each node as a Coord and each edge type as a CoordSet; adjacency reduces to a single bitwise AND over 175 machine words. Beyond set membership, the CoordCube view (§[sec-coordcube]) provides spatial region generation over the same coordinate space: proximity search (nodes within L∞ radius of a center), bounding box enumeration (nodes within a hyper-rectangle), and distance metrics (hamming, euclidean, manhattan). These are arithmetic operations on the existing byte array, not index scans — a capability unique to Tagma among identifier systems.
...
A `#![no_std]` library with an optional `alloc` feature (default: on). Without the alloc feature, only core types are available: `Coord`, `CoordPath`, `CoordSet`, and the single-Coord `CoordSpace`. With alloc enabled, the full multi-Coord map family is available. The library builds for native targets and `wasm32-unknown-unknown`.

#### Core Types

- **Coord**: a 16-bit newtype guaranteed valid in $[0, 11171]$. Bit-exact with the hardware decoder. Methods include three-axis decomposition `(initial, medial, final)`, composition, range validation, Hamming distance, and byte-order conversion. The `Display` trait renders a Coord as its corresponding Unicode character (U+AC00 + index), making encoded values human-readable without decoding.
...
| CoordSpaceN6 | 6 | $1.94 \times 10^{24}$ | Heap (lazy) | 5.91 ns | UUID-scale identity |
...
**`CoordSpace` (single-Coord, no allocator).** Backed by an inline $[\texttt{Option<V>}; 11172]$ array (22 KB for `Option<()>`, 89 KB for `Option<u64>`). Zero heap allocation. Every `Coord` is a direct array index: one load, no hashing, no collisions, no resizing. Full Entry API (`entry().or_insert_with()`), `drain()`, `retain()`, `iter_mut()`, `IntoIterator`, `Index<Coord>`, `PartialEq`, `Clone`, `Debug`.

**`CoordSpace2<V>` (N=2, dense heap).** Backed by a single `alloc_zeroed` of 119 MB ($V=()$) or 952 MB ($V=\texttt{u32}$). Single `Box` dereference, no hashing. Every two-Coord `CoordPath<2>` maps to a direct index into a flat $[\texttt{Option<V>}; 11172^2]$ slab. True Tagma throughout: one load, no collisions.
...
// Single-Coord dense: no allocator required, 0.39 ns
...
// Two-Coord dense: single alloc_zeroed, 0.39 ns
...
// Multi-Coord tree: compile-time depth N=6 (software fallback, 5.96 ns)
...
```rust
pub fn encode_u16(v: u16) -> [char; 2];         // u16 to two Unicode characters
pub fn encode_bytes(bytes: &[u8]) -> String;     // bytes to encoded string
...
- Lookup latency: native CoordSpace (dense array) is flat at **0.39 ns** across all depths — every Coord resolves to a single array load. The tree fallback (CoordSpaceN) scales linearly with depth: 2.69 ns at N=3, 58.6 ns at N=19 (SHA-256 scale, ↑**3.9x** vs SHA-256's 227 ns). Native CoordSpace reaches **582x** vs SHA-256. Recursive depth is bounded by schema, not data volume: $10^4$ and $10^{77}$ entries both cost $N$ dereferences in the fallback path, while the native dense path costs a constant 0.39 ns.
...
- Identity generation: SHA-256 lookup costs 227 ns; the tree fallback (CoordSpaceN) reaches $2^{256}$ at 19 Coords for 58.6 ns (↑3.9x). The native dense path (CoordSpace, CoordSpace2, CoordSpaceM3) holds at a flat 0.39 ns.
...
{{< include ../_include/bench/fig-bench-cube-vs-path.qmd >}}
...
- **Spatial query — two tiers**: (a) CoordSet bitwise AND resolves compound axis filters at 329 Melem/s -- 137x faster than HashMap scan. (b) **CoordCube on CoordSpaceN tree** generates proximity regions at 96-353 Melem/s and bounding boxes at 699 Melem/s. The tree scales as O(N) for single lookups (2.69 ns at N=3, 58.6 ns at N=19); CoordCube adds spatial query capability with only ~60 ns fixed overhead above the tree baseline, achieving near-native dense throughput without preallocating the full address space. The tree fallback becomes a spatial query engine.
...
|--------|---------|-----------------|-------------------|--------------------|--------------------|---------------------|
| Latency (ARMv8.4-A Firestorm) | 227 ns | 0.39 ns | 0.39 ns | 0.40 ns | 5.91 ns | 58.6 ns |
