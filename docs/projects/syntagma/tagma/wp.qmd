...
The following figures measure the second access channel: CoordCube-based proximity generation, bounding box enumeration, and distance metric throughput. All measurements are on ARMv8.4-A Firestorm using the same harness as the core lookup benchmarks.
...
Proximity generation throughput scales with output path count: 55 Melem/s at radius 0 (1 path) to 353 Melem/s at radius 5 (121 paths). Dimensional scaling follows output count (2r+1)^D rather than N alone. Distance metrics cost ~320 ps — below a single array lookup (390 ps) — because all computation is register-only with no memory access. KV proximity queries decompose into path generation (5% of total time) and CoordSpace tree traversal (95%).
