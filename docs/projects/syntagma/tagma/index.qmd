---
title: "Tagma"
subtitle: "Hashless spatial primitive on a fixed 16-bit, 3-axis Unicode composition space"
date: 2026-07
...
Tagma[^tagma-etym] is a computing primitive where the address *is* the coordinate — not a flat pointer, but a point in an N-dimensional geometric space. This is made possible by a fixed 16-bit contiguous Unicode block whose closed-form composition rule holds over *U+AC00..U+D7A3*, so every valid value is at once a 1-D address, a 3-D coordinate (Axis 0, Axis 1, Axis 2), and a compositional character. This triple interpretation gives a collision-free, hash-less, structurally addressable space [*-@sec-paradigm-shift*] and a single-cycle combinational decoder[^tagma-hw]. The coordinate arithmetic admits unbounded N-Coord composition — the address space grows as $11,172^N$ while lookup cost remains O(N) per Coord. We present a gate-level decoder specification, a processor pipeline attachment reference (e.g., RISC-V XIF)[^tagma-repo], a hardware reference implementation of all three primitive operations (decode, compose, and distance) verified over their input spaces against the Rust reference and formally equivalent to the synthesized netlist [*-@sec-hardware*], a software reference implementation [*-@sec-appendix-rust*], and a full benchmark suite [*-@sec-appendix-benches*]. Measured lookup latency in the software reference: 0.39 ns at single-Coord and 62 ns at SHA-256 scale (20 Coords), against SHA-256's 227 ns (582x and 3.7x). Structural prefix queries show the decisive advantage: a direct-address table answers a nonexistent prefix in 1.65 ns where a hash-table scan of 10M entries takes 23.05 ms (14.0Mx); bit-set axis filters resolve compound queries at 329 Melem/s against a 2.4 Melem/s scan (137x, bitwise AND against full scan); and 10M sparse gets complete in 44.9 ms against 1.05 s (23.4x). That standard is open, a public good whose composition rules carry no proprietary encumbrance. A production-scale validation in CERN's ROOT TTree read path confirms the model in petabyte-scale high-energy physics software [*-@sec-appendix-root-ttree*].
...
Reproduction: `make -C hw check` runs the whole gate (simulation, synthesis, and equivalence) from the sources recorded with this document's version, and the `hw` CI job runs the same gate. The HDL lives under `hw/` in the syntagma repository [@tagma-source].
...
The coordinate space is implemented as a multi-crate Rust reference implementation[^tagma-swref]. The `Coord` type is defined in the core library and is bit-exact with the hardware decoder. The workspace is published on GitHub [@tagma-source].
...
Conventional hardware searches. Tagma hardware interprets. This shifts the hardware design problem from faster computation to simpler decoding. The measured trade is explicit: timing closure costs about 2.5x gates [*-@sec-decoder*], and the N=1 dense case is examined in Section [*-@sec-memory-hierarchy*]. All three operations of the primitive are realized and verified in RTL; the realization and its remaining work are stated in Section [*-@sec-hardware*].
...
[^tagma-repo]: Repository: [Github (Pre-release)](https://github.com/ssccsorg/syntagma), Benchmark [github.com/ssccsorg/syntagma/blob/main/sw/rust/benches/bench.rs](https://github.com/ssccsorg/syntagma/blob/main/sw/rust/benches/bench.rs)
...
[^tagma-root-ttree]: Solving CERN's ROOT TTree I/O bottleneck: [docs.ssccs.org/works/cern/root-ttree/](https://docs.ssccs.org/works/cern/root-ttree/); measured evidence reproducible from the [ROOT fork](https://github.com/ssccsorg/root), its [benchmark](https://github.com/ssccsorg/root/blob/master/benchmarks/tagma/tagma_bench.C), and the [measured artifact](https://github.com/ssccsorg/root/blob/2fc1c8204bdeefcc82c3a34cf4c693ff19906eb5/benchmarks/tagma/result/reference.json) that records every row.
