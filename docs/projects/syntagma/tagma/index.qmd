  ...
  pdf:
    ...
    include-in-header:
      text: |
        ...
        \IfFileExists{./_files/tagma_metadata.tex}{\input{./_files/tagma_metadata.tex}}{\GenericWarning{}{Metadata: no generated file is present, so the version stamp and the affiliation values are empty. Run 'sdb pre .' to write them.}\providecommand{\affiliationname}{}\providecommand{\affiliationurl}{}\providecommand{\affiliationdomain}{}\providecommand{\authoremail}{}\providecommand{\authorrole}{}}
        ...
        \def\@maketitle{
          \begin{center}
            ...
            \vspace{2em}
                {\normalsize \@date \par}
                \vspace{0.3em}
                {\normalsize \texttt{\href{https://doi.org/10.5281/zenodo.21302508}{doi.org/10.5281/zenodo.21302508}} \par}
          \end{center}
          \vspace{2em}
        }
        \makeatother
...
## Hardware Realization {#sec-hardware}

This part states the hardware realization of the primitive on its own terms, so that it can be read without the software benchmark sections above. The same formula the Rust reference evaluates in arithmetic is evaluated here in gates, and every claim in this part is reproduced by the hardware verification gate in the syntagma repository under `hw/`.

### The primitive in gates

The primitive is three combinational functions over the same 16-bit value, each a separate module verified independently:

| Operation | Module | Input | Output | Cells (2-input) |
|-----------|--------|-------|--------|------------------|
| decode | `tagma_decoder` | 16-bit code point | three axes, `valid` | 588 |
| compose | `tagma_compose` | three 5-bit axes | 14-bit coordinate index, `valid` | 258 |
| distance | `tagma_dist` | two 16-bit code points | three 5-bit distances | 1329 |

Decode and distance take a code point; compose returns the coordinate index, the offset from U+AC00, which is the internal representation of the Rust `Coord`, so the code point is U+AC00 + index. The distance unit instantiates two decoders, one per operand, which is why it is the largest of the three. A fourth module, `tagma_segment_store`, realizes the address path as an 11,172-slot store and is described under Memory hierarchy penetration below.
...
The ~300 gate claim of earlier documents holds for the naive shift-subtract decoder (206 to 232 cells), but timing closure on a 12 MHz board requires the multiply-shift structure at about 2.5x the gate count. The current RTL implements the constant divisions with multiply-shift, exact over the valid domain: division by 28 becomes division by 7 on `offset >> 2` (at most 2792), and division by 21 applies to values at most 398. Both structures are functionally identical, which the verification pipeline proves over all 2^16 inputs. The decoder is verified exhaustively over all 11,172 valid characters in three simulation channels (formula, Rust reference, gate netlist), plus formal equivalence over all 2^16 inputs. The full compliance criteria for Tagma-compatible implementations are defined in Section [*-@sec-compliance*].
...
All three operations are realized in the hardware tree (`hw/rtl/`) as the modules `tagma_decoder`, `tagma_compose`, and `tagma_dist`, each verified in the same four channels as the decoder. The decode and distance units take a 16-bit code point; the compose unit returns the coordinate index, the offset from U+AC00, which mirrors the Rust `Coord`. In a 2-input gate library the three units are 588, 258, and 1329 cells; the distance unit carries two decoder instances, because a pair needs both operands decoded.
...
The hardware realization closes the loop against the Rust reference with four independent channels: formula simulation, golden anchors exported from `Coord::to_axes`, gate-level netlist simulation, and formal equivalence of the RTL against the synthesized netlist over all 2^16 inputs. All three simulation channels pass all 11,172 valid characters. The exhaustive tests surfaced the boundary correction: the last valid character is U+D7A3, not U+D7AF, and the testbench boundary was wrong initially. The pipeline is reproducible from a clean checkout with `make -C hw check` in the syntagma repository, and runs in the `hw` CI job.

The three units cover their own input spaces in the same channels:

| Unit | Simulation | Formal equivalence | Gate-level cells |
|------|------------|--------------------|------------------|
| decoder | all 11,172 valid characters | all 2^16 inputs | 588 |
| compose | all 32^3 axis combinations | all 2^15 axis combinations | 258 |
| distance | 11,172 code points against two references and a varying-operand sweep | all 2^32 input pairs | 1329 |
| segment store | 11,172 slots, reserved addresses, read-during-write (behavioral model) | not applicable | not applicable |
...
A hardware implementation is Tagma-compatible iff, in addition to the conditions in Section [*-@sec-compliance*], it satisfies:

1. **Decode bit-exactness.** Decode is a combinational function bit-exact with `Coord::to_axes` over all 2^16 inputs, reporting validity for the 11,172 valid values.
2. **Compose bit-exactness.** Compose is bit-exact with `Coord::from_axes`: it returns the coordinate index for an in-range axis triple and reports invalidity otherwise.
3. **Distance bit-exactness.** Distance is bit-exact with `Coord::hamming_distance` for valid operand pairs.
4. **Netlist equivalence.** The RTL and the synthesized netlist are formally equivalent over the full input space of each operation.
...
Reproduction: `make -C hw check` runs the whole gate (simulation, synthesis, and equivalence) from the sources recorded with this document's version, and the `hw` CI job runs the same gate. The HDL lives under `hw/` in the syntagma repository [@tagma-source] and is licensed under CERN-OHL-P v2; the software reference under `sw/` remains Apache 2.0, and this document's text is CC BY-NC 4.0.
...
**Radiation-tolerant computing.** The structural validity check embedded in every decode provides inherent error detection. A single-bit upset that maps a valid coordinate outside the U+AC00-U+D7A3 range produces an immediate invalid flag. Errors that map one valid coordinate to another are not detected by the structural check alone and require ECC supplementation, as noted in Boundaries. The decoder's small measured gate count (478 cells in the ev-compatible generic schema, 588 in a 2-input gate library) makes triplication feasible at lower cost than protecting a full hash unit.
...
Tagma defines a new type of silicon primitive: a combinational decoder that derives identity from structure at a measured 478 cells (generic schema; 588 in a 2-input gate-level estimate) and one cycle. At this gate cost, content-addressable operation becomes viable where hash-based approaches are too expensive in power, area, or latency. The decoder is verified exhaustively over all 11,172 valid characters in three simulation channels, plus formal equivalence to the synthesized netlist over all 2^16 inputs; the compose and distance units are verified the same way. The structural validity check embedded in every decode provides inherent error detection, relevant for radiation-tolerant computing in space environments. The coordinate space that enables this is this Unicode block, an open international standard and a public good. Tagma is released as open-source hardware under CERN-OHL-P v2 in `hw/` in the syntagma repository, with the FPGA demo closing the 12 MHz board clock at 16.79 MHz, inviting the next conversation: what else becomes possible when identity costs less than a single multiply. The broader hardware design implications of this shift are discussed in Appendix [*-@sec-appendix-hardware*].
...
Conventional hardware searches. Tagma hardware interprets. This shifts the hardware design problem from faster computation to simpler decoding. The measured trade is explicit: timing closure costs about 2.5x gates [*-@sec-decoder*], and the N=1 dense case trades per-operation speed for energy, determinism, and physical embedding [*-@sec-memory-hierarchy*]. All three operations of the primitive are realized and verified in RTL; the remaining hardware work is the OpenRAM macro for the segment store, the physical FPGA board bring-up, and the power model over the VCD activity trace. The realization itself is stated in Section [*-@sec-hardware*].
...
## TagmaMatrix[^tagma-matrix]: Coordinate-Addressed Matrices {#sec-appendix-tagma-matrix}
...
The full design and the verification surface are described in the TagmaMatrix whitepaper; the reference implementation lives in `sw/rust/matrix`[^tagma-matrix-src].
...
[^tagma-matrix]: TagmaMatrix whitepaper (draft): [docs.ssccs.org/projects/syntagma/tagma/matrix](https://docs.ssccs.org/projects/syntagma/tagma/matrix)
[^tagma-matrix-src]: TagmaMatrix reference implementation: [github.com/ssccsorg/syntagma/tree/main/sw/rust/matrix](https://github.com/ssccsorg/syntagma/tree/main/sw/rust/matrix)
