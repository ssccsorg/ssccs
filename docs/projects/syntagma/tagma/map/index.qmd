...
|---------|---------|-------|------------|
| **CoordMap2** | N=2 dense | **18.69ns** | **2.4x faster** |
| CoordMapN<2> | N=2 tree | 18.88ns | 2.4x faster |
| DynCoordMap | trie | 49.44ns | 1.1x slower |
| **HashMap** | SipHash | 44.87ns | baseline |

| KV Type | Backend | ns/op | vs HashMap |
|---------|---------|-------|------------|
| **CoordMap2** | = | **22.06ns** | **1.08x faster** |
| CoordMapN<2> | = | 21.73ns | 1.09x faster |
| DynCoordMap | = | 42.40ns | 1.78x slower |
...
| CoordMapN\<2\> | 22.0 ns | 21.9 ns | 22.1 ns | flat | 1.08x faster |
...
The table exposes the hierarchy directly: Native mode is 60x faster than HashMap for single gets. CoordMap2 mode is 1.08x faster than HashMap for single gets and 10.1x faster for batch gets. After conversion, spatial queries cost 94 ns regardless of mode. HashMap provides no spatial queries without secondary structures.
...
```rust
use tagma_kv::{CoordMap, CoordMap2, CoordMapKey};
use tagma_kv::coord_kv::CoordMap;
use tagma_kv::dyn_coord_kv::DynCoordMap;
use tagma_kv::coord_kv_n::CoordMapN;
