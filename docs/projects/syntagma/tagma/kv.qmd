...
The full axis query API (axis slicing, range queries, compound masks) is implemented as the `SpatialKV` trait on top of the same CoordKV trait foundation. Given a center `CoordPath<N>` interpreted as a `CoordCube<N, D, R>` (N = D × R), the `proximity` method generates all paths within L∞ radius and returns stored entries:
