...
version-prefix: tagma-kv-10.5281/zenodo.21499437
version-mark: true
...
Tagma[^tagma] is a spatial coordinate computing system. It is not a hash map. Its address space is a geometric coordinate system where every valid coordinate is deterministic, collision-free, and structurally decomposable into independent axes. Tagma and hash-based systems (HashMap, hash tables, DHTs) solve different problems: hash functions distribute arbitrary inputs uniformly across buckets under probabilistic collision; Tagma assigns every key a fixed position in a geometric space with zero collision probability. They are complements, not substitutes. Tagma-KV [^syntagma] [^syntagma-repo] provides a HashMap-compatible API (insert, get, remove, contains_key) backed by Tagma's coordinate space. It exists so that existing KV-oriented code can access Tagma's spatial capabilities without architectural rewrites. The cost is the conversion from legacy key types (&str, Vec<u8>) to Tagma coordinates.

Benchmarks[^benchmarks] show that this conversion cost is 5 ns (str-to-CoordKey byte copy) — approximately 3x faster than SipHash-2-4. The remaining 16.67 ns of the 21.67 ns total get latency is the Vec clone for the owned return value, an API artifact that reference-returning variants can eliminate. The critical finding is that CoordKV2 latency is scale-invariant: 21.5 ns at 10k, 1M, and 10M operations, because the CoordSpace2 dense array provides uniform access latency regardless of dataset size. HashMap per-op cost rises 19% from 10k to 10M as the working set exceeds cache capacity, and this trend continues with scale. The conversion is a one-time cost: once paid at the entry boundary, subsequent accesses via CoordKey skip it entirely, and CoordKey is a 128-bit Copy type that can be cached at zero marginal cost. HashMap has no equivalent amortization — it pays SipHash on every operation with no bypass. For systems that use native coordinates directly, the access cost drops to the physical limit of array load (0.39 ns, 60x below HashMap).
...
| Layer | Cost | Description | Payment model |
|-------|------|-------------|---------------|
| `str → CoordKey<2>` conversion | 5 ns | byte copy + bounds check | **one-time** |
| `Box<[u8]> → Vec<u8>` clone | 16.67 ns | heap allocation + value copy | **per-get** |
| CoordSpace2 slot load | 0.39 ns | direct array index, physical limit | **per-get** |
| **CoordKV2 total** | **22.06 ns** | — | — |

| Layer | Cost | Description | Payment model |
|-------|------|-------------|---------------|
| SipHash-2-4 | ~15 ns | per-byte compression | **per-get** |
| Bucket probe | ~5 ns | index extraction + chain walk | **per-get** |
| Return ref + collision | ~3.79 ns | collision resolution | **per-get** |
| **HashMap total** | **23.79 ns** | — | — |
...
1. **Conversion cost derivation**: Total CoordKV2 get latency is 22.06 ns. Raw CoordSpace2 array access is 0.39 ns (L1 cache load latency on ARMv8.4-A Firestorm Firestorm core). The remaining 21.67 ns splits into the str-to-CoordKey conversion (~5 ns) and the Vec clone for the owned return value (~16.67 ns). The conversion alone is below SipHash's cost; the Vec clone is an API artifact that reference-returning variants can eliminate.
...
| CoordKV2 (string-key) | 21.67 ns | 0.39 ns | 22.06 ns | 22.0 ns | 21.4 ns | 21.5 ns | 94 ns |
...
**Remark.** The structural coordinate space that enables this is derived from a fixed 16-bit Unicode block (U+AC00–U+D7AF). The block encodes the Hangul writing system, which provides the 19, 21, and 28 axis ranges. The full specification, including the composition formula and compliance criteria, is defined in the Tagma whitepaper [@tagma-wp].
...
CoordCube extends this by generating **sets of CoordPaths** from a spatial region specification (proximity radius or bounding box).  CoordCube does **not** store data — it generates paths for the existing tree to look up.  The `CoordCubeKV` trait pairs path generation (via `tagma-geo`) with tree lookup (via `CoordKV`) in a single call:
...
The full axis query API (axis slicing, range queries, compound masks) is implemented as the `CoordCubeKV` trait on top of the same CoordKV trait foundation. Given a center `CoordPath<N>` interpreted as a `CoordCube<N, D, R>` (N = D × R), the `proximity` method generates all paths within L∞ radius and returns stored entries:

```rust
use tagma_kv::spatial::CoordCubeKV;

let center = CoordPath::<2>::new([c0, c1]);
// Interpret as D=2, R=1: a 2D cube with one syllable per dimension
let results: Vec<(CoordPath<2>, Vec<u8>)> = kv.proximity::<2, 1>(&center, 1);
```

The `bounding_box_range` method accepts per-syllable min/max ranges:
...
Both methods combine CoordCube path generation (55-700 Melem/s) with CoordSpace tree lookup. Path generation accounts for approximately 5% of total query time; the remainder is tree traversal and result collection.
