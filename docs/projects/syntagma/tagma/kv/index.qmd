...
Tagma-KV supports a two-phase access pattern. Data enters through the KV interface: `insert("key", value)` converts the string key to a CoordPath once and stores it in the CoordSpace. Subsequent reads bypass the KV layer entirely, operating on the CoordPath or Coord directly:

```rust
// Phase 1: load through KV interface (one-time conversion)
kv.insert("user:42", payload);

// Phase 2: query through Tagma native layer (zero additional conversion)
let coord = Coord::new(index).unwrap();
let axes = coord.to_axes();                        // 3-axis decomposition
let membership = coords.contains(&coord);           // CoordSet membership
let intersection = set_a & set_b;                   // bitwise compound query
```

The conversion cost is paid exactly once at write time. Every subsequent read -- structural decomposition, set membership, spatial filter -- operates at Tagma's native speed with no further KV overhead.

The two modes compose: load data through Mode B (KV insert with string keys), then query through Mode A (native CoordPath access for spatial operations). The conversion cost is paid once at write time; every subsequent read benefits from Tagma's full structural capability at zero additional cost.
