...
A hardware decoder extracts the three axis fields in one cycle. Of 65,536 possible 16-bit values, 11,172 are structurally valid and the remaining 54,364 are immediately detectable as invalid. CoordPath chains multiple Coords into higher dimensions (6 = 18 axes, 19 = 57 axes), with each axis position representing a different application-defined dimension — region, device type, timestamp, shard. The arithmetic operations are invariant; only the axis interpretation changes per deployment.

No other Unicode script satisfies all four conditions required: a contiguous fixed-width 16-bit address range, a closed-form composition formula, a complete three-axis decomposition, and an open international standard. This block (Hangul) is unique.
...
Spatial query: CoordSet bitwise AND resolves compound axis filters at 329 Melem/s — 137x faster than HashMap scan. Edge: CS2 sparse get sustains 23.4x at 10M entries; CoordSpaceN19 get shows 19-dereference cost (0.50x); drain is 0.72x on the full space.
 The CoordCube (§[sec-coordcube]) adds a second query channel: spatial region generation over the same CoordPath bytes, with zero additional memory overhead — all operations are register-only arithmetic. Proximity search on the CoordSpaceN tree achieves 96-353 Melem/s; in sparse stores, Tree+Cube is 3.3x faster than sequential Path lookups (48 ns vs 159 ns for 9 keys); bounding boxes reach 699 Melem/s. Distance metrics cost ~320 ps — below a single CoordSpace lookup (390 ps). KV proximity queries decompose into path generation (5% of total time) and CoordSpace tree traversal (95%), with end-to-end throughput of 31-584 Melem/s depending on store density. CoordCube does not modify the tree — it generates paths for the existing tree to look up.
:::{ layout-ncol=2 }
...
Bulk operations: CoordSpace outperforms HashMap by 14.6–17.3x across all operations on the full 11,172-entry space. Single-get microbenchmark isolates the per-operation cost: 0.82 ns vs 8.9 ns.
...
[^hangul-coord-structure]: The constants 19, 21, and 28 correspond to the number of initial consonants, medial vowels, and final consonants in Hangul (invented 1443), the writing system encoded in block U+AC00--U+D7AF.
