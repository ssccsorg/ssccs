...
FihStorage realizes this model as a structural index over the Tagma coordinate space. After the 2026-08 restructure, record identity is a full-injective CoordId<20> that carries all 256 bits of identity, and a content-hash guard rejects id collisions at commit time. Any backend that stores an append-only record, a stateful record, and a read-only record is sufficient, from SQLite to object storage. The same binary runs on Wasm, edge nodes, portable devices, blockchain runtimes, and bare-metal containers.
...
### Restructure measurements (2026-08)

The 2026-08 restructure, measured on the same Apple M1:

- Content-hash conflict check: 135-390 ms to 445 ns per call.
- Per-fact footprint: ~3.4 MB to ~40 KB average.
- Stress scenario (10,000 events): 17.7 s to ~9.0 s after state reads moved to in-memory indexes.
- Hundreds of automated tests passing.

## Resource profile

The coordinate tree allocates one leaf node per written prefix, and each leaf covers 11,172 slots regardless of occupancy, a fixed cost that trades memory for predictable performance, unlike hash tables that grow unpredictably. Memory is bounded by axis cardinality, not record count; the 2026-08 restructure cut the per-fact footprint from ~3.4 MB to ~40 KB average. Dense packing removes allocation entirely at a fixed memory floor and holds a flat 6 ns lookup. Graph traversal, gap detection, and reporting operate on accumulated facts at zero marginal inference cost; AI use is confined to knowledge-branch generation.
...
- **Restructured record layer (2026-08)**: record identity is now a full-injective 256-bit coordinate; structural queries run over a compact coordinate index; content-hash conflict detection hardens the record layer.
...
- **Faster state reads**: state reads now serve from in-memory indexes; the 10,000-event stress scenario dropped from 17.7 seconds to 9.0 seconds.
...
- A benchmark-validated multi-dimensional search path over the coordinate index.
...
[^old]: Pre-Tagma write baseline from the `Vec<FactRecord>` full-scan implementation, measured on the same Apple M1. Devlog 160 (2026-07-29): <https://github.com/ssccsorg/nexus/blob/main/docs/2026-07-29-160-syntagma-integration.md>
