...
The query side shows the same flatness. On a time-bounded three-axis filter (origin, creator, time range), the coordinate tree resolves the query 6x to 25x faster than the record-map scan at 100k facts, and 5x to 47x faster at 1m facts. The gap widens as records accumulate because the scan pays the full store on every query while the tree pays only the subtree. The same behavior appears on a real document set with the full FIH lifecycle (add, intent, claim, conclude): the tree advantage grows from 2.4x at phase 1 to 2.9x at phase 3 as the knowledge network accumulates.
...
- **Flat indexing cost measured**: the structural filter index holds a constant footprint as records grow; multi-axis queries via the coordinate tree run 5-47x faster than the record-map scan at 1m facts, with the gap widening on accumulation.
