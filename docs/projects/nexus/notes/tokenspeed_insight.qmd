...
neXus's Phase 4 agentic research loop involves a Planner that generates hypotheses, an Executor that runs experiments, and a Verifier that validates results before artifact generation. The current contract-governed workflow relies on `contract` schemas validated at submission time — a runtime check.
