...
Because of this, nexus makes async the design center rather than an adapter bolted on afterward. The IO layer is async at the trait level, and sync callers use a blocking wrapper that calls `futures_executor::block_on` internally. This is not an adapter layer — async is primitive, sync is extension.

This design aligns naturally with every target platform. R2 and R3-like storages support `await` directly with no `block_on` needed. On tokio, the pattern is `spawn` plus `await` on async filesystem and network operations. On wasm32, single-threaded cooperative multitasking works through `await` alone. Each platform uses the same async trait; only the executor changes.

The blocking wrapper overhead is negligible because the core storage runtime buffers all writes into pending operations and only flushes in batch. Rather than each `set()` call traversing the I/O boundary independently, individual calls are amortized across the flush cycle.
...
| **Foundation (IO Layer)** | Stable | Existing comprehensive test suite; core storage runtime/DuckDB contract; never modified without a nexus-sim scenario proving the capability gap | StorageRead, FactCapable, IntentCapable, HintCapable; DualStorage; core storage runtime/R2 bindings |
...
- `nexus/` — `DefaultBlackboard`: the single `Blackboard` implementation backed by `DualStorage`, plus the core storage runtime, `PetgraphStorage`, `CompositeColdStorage`, OODA scheduler, and all detectors. (Consolidated from former `core/graph/`, `core/process/`, `core/storage/petgraph/`, `core/storage/composite/`.)
...
Phase 3 (native layered coordinate model storage) is complete: a comprehensive test suite across the core storage runtime, incremental persistence, ColdStorage trait, and temporal query validation. The core storage runtime (coordinate-based index with temporal ordering) replaces PetgraphStorage as the primary hot storage.

| Component | Status |
|---|---|
| Semantic store trait + lookup handle | Implemented (core storage module) |
| Semantic index slot in coordinate index | Implemented (coordinate index) |
| In-memory mock semantic store (brute-force cosine) | Implemented with unit tests |
| External semantic backends (vector, bm25, ngram) | Future (external crates) |
...
| `nexus/` crate | `DefaultBlackboard` (single crate: core storage runtime + `PetgraphStorage` + `CompositeColdStorage` + OODA scheduler + all detectors) |
| `storage/duckdb/` crate | `DuckDbStorage` with `DuckDbQueryExt` (CTE, window, JSON, vector search) |
| Core storage runtime (hot, primary) | Coordinate-based index with temporal ordering, native layered coordinate model (`FactCapable` + `IntentCapable` + `HintCapable` + `EvictCapable` + `TimeRangeCapable`) |
| PetgraphStorage (hot, optional) | In-memory 2D graph projection (`FactCapable` + `IntentCapable` + `HintCapable` + `EvictCapable`) |
...
| Scenario tests | Extensive test suite across numerous test files |
| Core storage runtime tests | Comprehensive suite (Phase 3 complete, #86) |
| Incremental persistence | Implemented: incremental delta aggregation with cursor tracking |
| ColdStorage trait | Implemented: unified interface for cold backends |
| Temporal query validation | Implemented: temporal query replay and consistency checks |
