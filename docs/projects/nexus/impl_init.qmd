...
digraph NexusUnified {
    ...
    graphlayer [label="Memgraph\n(Graph DB + Proxy)", shape=cylinder, style=filled, fillcolor="#dda0dd"];
    modulehub [label="Module Hub\n(analysis orchestration)", shape=box, style=filled, fillcolor="#e6ffe6"];
    graphlayer -> modulehub [label="Cypher queries"];
    ...
    graphrag [label="RAG Engine", shape=cylinder, style=filled, fillcolor="#ffe4e1"];
    graphrag -> graphlayer [label="GraphML export"];
...
| Layer | Component | Stack | Role | Status |
|-------|-----------|-------|------|--------|
| **RAG** | LightRAG | Python/Docker | Document ingest, entity extraction, semantic search | Deployed |
| **RAG** | Graphiti | Python/Docker + Neo4j | Temporal knowledge graph | Deployed |
| **Graph** | Memgraph | C++/Docker (in‑memory) | Persistent graph DB, Cypher analysis, native vector search, community detection (Leiden/Louvain), PageRank | Deployed |
| **Graph** | memgraph-proxy | Python HTTP → Bolt | HTTP‑to‑bolt bridge for Worker access | Deployed |
| **Infra** | Cloudflare Workers | TypeScript | af-sync (R2→RAG sync), module-hub (orchestration) | Deployed |
| **Infra** | Cloudflare Tunnels | cloudflared | Public access to local Docker services | Deployed |
...
### Graph Analysis Layer — Memgraph

Alongside the RAG engines, Nexus operates a separate graph analysis layer based on [Memgraph](https://memgraph.com/) — an in‑memory graph database with native vector search. Unlike external vector databases that require a separate index, Memgraph integrates vector similarity directly into Cypher queries, enabling combined structural + semantic retrieval in a single query.

The gap-detector module (`module-hub`) runs periodic structural analysis of the knowledge graph via Cypher queries:

| Analysis Type | Cypher Query | Severity |
|--------------|--------------|----------|
| **Orphaned concepts** | `MATCH (c:Concept) OPTIONAL MATCH (c)-[r]-() WHERE rc = 0` | Warning |
| **Missing relationships** | `MATCH (a) MATCH (b) WHERE a.file_path = b.file_path AND NOT EXISTS ((a)-[]-(b))` | Info |
| **Contradictory edges** | `MATCH (a)-[r1]->(b) MATCH (a)-[r2]->(b) WHERE type(r1) <> type(r2)` | Critical |

Memgraph is accessed by Workers via an HTTP‑to‑Bolt proxy (`memgraph-proxy`) that translates HTTP POST `/cypher` requests to the Memgraph bolt protocol. The proxy runs as a Docker sidecar alongside Memgraph.

**Architecture:**

```text
LightRAG (RAG ingest)
  → GraphML export → import_lightrag.py
Memgraph (graph storage + Cypher analysis)
  ↑ HTTP proxy (:7689)
module-hub gap-detector
  → findings → KV
```

Access to the graph layer is through a dedicated Cloudflare Tunnel:

| Tunnel | Hostname | Service |
|--------|----------|---------|
| `nexus-graph` | `graph-api-dev.nexus.ssccs.org` | Memgraph proxy (:7689) |

This isolation ensures that graph analysis operates independently from the RAG ingest pipeline — a decision driven by the recognition that graph structural analysis and document retrieval have fundamentally different latency, consistency, and update patterns.
...
| Component | GraphRAG | Memgraph | R2 | Sync Worker | Module Hub | Planner | Verifier | Generator |
|-----------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| GraphRAG | — | → GraphML export | — | ← syncs | — | ← queries | ← grounds | — |
| Memgraph | ← import | — | — | — | → Cypher queries | — | → evidence | — |
| R2 | — | — | — | ← reads | → artifacts | — | — | — |
| Sync Worker | → DELETE/POST | — | → list/get | — | — | — | — | — |
| Module Hub | — | → gap analysis | — | — | — | — | — | — |
| Planner | → queries | — | — | — | — | — | → delegates | — |
| Verifier | → hybrid queries | → support score | — | — | — | ← receives | — | → signals |
| Generator | — | — | — | — | — | — | ← triggered | — |
