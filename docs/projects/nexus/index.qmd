...
neXus is a self-evolving, modular, contract-governed agentic research infrastructure. An ultra‑thin, decentralized binary hub of just a few megabytes. Attach compact or enterprise‑scale storage to this core and capacity scales with completely linear incremental costs. The same Wasm bytecode ports without modification to any edge node, portable device, blockchain, or system container that supports a Wasm or bare‑metal runtime. Every agent (embedded sensor, robot, edge worker, or cloud cluster) runs the same hub, connecting to any backend (object storage, relational database, key-value store, RTC channel, Bluetooth mesh, or embedded filesystem) that implements the three primitive operations. Complexity and scaling are delegated to the external storage and communication tiers that implement the protocol.
