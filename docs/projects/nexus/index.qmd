---
title: neXus
subtitle: A swarm computing runtime fabric for autonomous execution.
date: last-modified
metadata-files:
  - ../../_include/author.yml
abstract: |
  Modern fundamental computing research redefining the von Neumann
...
neXus is a minimal, decentralized runtime orchestration platform and its spec for autonomous entities. Its core runtime called nex is a lightweight binary hub—a few megabytes—that scales by attaching storage, from embedded filesystems to enterprise databases. The same binary runs on Wasm, edge nodes, portable devices, blockchain runtimes, or bare-metal containers. Any backend that can store an append‑only record (Fact), a stateful record (Intent), and a read‑only record (Hint) becomes a fully functional neXus Blackboard without a graph database or specialised indexing layer. Serverless deployment is a natural consequence of the data model, not a retrofit.
...
LLMs are the most expensive and least predictable component in any agentic system. neXus confines their use to the precise moment a new knowledge branch must be opened. Once a Fact is committed, querying it, traversing the graph, detecting gaps, and generating routine reports runs entirely on accumulated knowledge with zero inference cost. Over time, as the Fact graph deepens, the LLM fraction approaches zero. The system compounds its own understanding without recurring cost.
...
neXus supports N deployment variants. Two representative cases: the **real-time Blackboard** is a persistent daemon that keeps the FIH graph in memory for sub-millisecond reads and live subscribe-based notifications; on graceful shutdown it flushes state to durable storage. The **storage / knowledge / Fact store** is a serverless worker backed by R2 or equivalent object storage, running on demand with no subscribe capability. Both variants expose the same FIH interface and can be linked: a real-time Blackboard uses a storage variant as its durable backend.
...
The storage layer beneath this economy is deliberately heterogeneous. The
same Fact graph, the same cursor, and the same FIH primitives operate
identically across local files, object storage, and blockchain backends.
Storage is a pluggable trait; the core never changes. A researcher on a
laptop, a server in a data centre, and an on-chain contract all read from
and write to the same verified knowledge graph through the same interfaces.
