...
neXus is a self-evolving, modular, contract-governed agentic research infrastructure. The design targets an ultra‑thin, decentralized binary hub of just a few megabytes. Heterogeneous porting will expand gradually: starting from a basic Wasm target, the compiler orchestrator will incrementally optimize and extend native compilation across edge nodes, portable devices, blockchain runtimes, and bare‑metal system containers. Attach compact or enterprise‑scale storage to this core and capacity scales with completely linear incremental costs. Every agent (embedded sensor, robot, edge worker, or cloud cluster) runs the same hub, connecting to any backend (object storage, relational database, key-value store, RTC channel, Bluetooth mesh, or embedded filesystem) that implements the three primitive operations. Complexity and scaling are delegated to the external storage and communication tiers that implement the protocol.

Within this footprint, the architecture is designed to host 100,000 autonomous agents operating concurrently — designing silicon chips, researching within closed‑loop environments, and manufacturing the results. As this paradigm expands into robotics, the goal is an entirely automated, closed‑loop machine ecosystem.
...
The primary document builder that feeds this infrastructure is the [SDBS (SSCCS Document Build System)](/projects/sdbs/index.html). SDBS is a parallel build pipeline that validates, renders, and publishes all technical documentation. neXus reads from the resulting structured corpus, using the document graph as a partial knowledge base from which hypotheses, constraints, and provenance traces are derived.
