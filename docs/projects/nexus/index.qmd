...
neXus is a homeomorphic runtime fabric unifying swarm agents and spatial storage through contract-governed protocol over an immutable problem-knowledge-solution space. Its core runtime called `nex` is a lightweight binary hub that scales by attaching storage or any agent orchestration from embeded to enterprise server environment, like a tiny universal USB hub which has memory and processing unit on the same device package.
