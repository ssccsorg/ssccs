...
neXus supports N deployment variants. Two representative cases: the **real-time Blackboard** is a persistent daemon that keeps the F-I-H graph in memory for sub-millisecond reads and live subscribe-based notifications; on graceful shutdown it flushes state to durable storage. The **storage / knowledge / Fact store** is a serverless worker backed by R2 or equivalent object storage, running on demand with no subscribe capability. Both variants expose the same F-I-H interface and can be linked: a real-time Blackboard uses a storage variant as its durable backend.
