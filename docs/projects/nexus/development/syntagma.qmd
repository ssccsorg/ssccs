...
abstract: |
  ...
  about 901 bytes per fact, and time-bounded multi-axis queries resolve 5x to 47x
...
The query side shows the same flatness. A time-bounded three-axis filter (origin, creator, time range) resolves 6x to 25x faster through the coordinate tree than through the record-map scan at 100k facts, and 5x to 47x faster at 1m facts. The scan pays the full store on every query; the tree pays only the subtree, so the gap widens as records accumulate. On a real document set with the full FIH lifecycle (add, intent, claim, conclude), the tree advantage grows from 2.4x at phase 1 to 2.9x at phase 3.
...
The 2026-08 multi-dimensional benchmark resolves the time range through the leading time axes instead: an `iter_prefix` day walk over `[time_hi, time_lo, entity, origin, creator]` turns a bounded time window into O(window days) subtree navigations, measured at 5x to 47x over the record-map scan at 1m facts (Flat Indexing Cost section). The wiring of that path into the filtered read path is the recorded decision; the B-tree and `iter_range` options below remain alternatives for unbounded or arbitrary ranges.
