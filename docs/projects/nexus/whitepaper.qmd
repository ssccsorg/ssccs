---
title: "neXus"
subtitle: "A heterogeneous, horizontal, coordinate-addressed runtime network for agents and executable systems from edge to bare metal."
date: 2026-09
date-format: "MMMM, YYYY"
version-prefix: nexus-doi.org/10.5281/zenodo.22303887
version-mark: true
colorlinks: true
latex-clean: false
metadata-files:
...
As the first program in a new language is often a calculator, `nex-calc`[^nexcalc] is the calculator of neXus: deliberately slow, and a minimal core proof. `put 3` writes a Fact at a coordinate, `put 5` writes another, `add` creates an Intent that is a directional vector between Facts, and `resolve` traverses from Fact to Intent to Fact and produces a new persistent Fact. The algebraic shape of the skeleton is
...
### Identity Lookup and Tree Walk {#sec-perf-identity}
...
### Knowledge Base and Write Path {#sec-perf-kb}
...
A separate axis of comparison is the orchestration harness. Recent agent frameworks coordinate model calls through a fixed program: the harness owns the state machine, lays out the graph or pipeline of steps, and routes messages between roles. neXus inverts this shape. There is no harness-owned state machine and no fixed program; the fabric is a concurrent substrate on which observers move. An observer is any executor in the kineTics sense, an agent, a sensor, a hardware transducer, or a human-operated terminal, and its only commitment is a set of Hints, the contract that bounds what it may do. Under that contract it reads and writes coordinate-addressed records on the shared blackboard, and coordination between observers is stigmergic: each acts on the traces the others left, with no central router deciding the next step.

| Dimension | Orchestration harness | neXus fabric |
|---|---|---|
| Coordination | central harness runs a fixed graph or pipeline | none; observers coordinate through shared immutable records |
| State | harness-owned session or context object | the record is the state; the blackboard is append-only |
| Agent contract | tool and model calls defined by the harness | Hints bound what an observer may do; roles are not assigned |
| Retrieval | vector or external index chosen by the harness | coordinate addressing via Tagma; no inverted index for lookup |
| Memory | per-run context or external store | FIH records over any backend, decoupled from the platform |
| Scale | one orchestration run | horizontal fabric of independent nex instances |

The difference is not throughput but control. A harness composes a fixed sequence and is therefore deterministic by construction, which is its strength for reproducible single workflows; neXus does not sequence observers and cannot guarantee a step order, which is its strength for open-ended concurrent accumulation. The two are complementary: a harness may run inside an observer, and the trace that observer leaves is recorded as Facts like any other, but the harness does not become the coordination medium of the fabric.
...
[^repo]: neXus reference repository, [github.com/ssccsorg/nexus](https://github.com/ssccsorg/nexus), Apache License 2.0.
[^nexcalc]: `nex-calc` source, [github.com/ssccsorg/nexus/tree/main/apps/nex-calc](https://github.com/ssccsorg/nexus/tree/main/apps/nex-calc)
[^synTagma]: synTagma is the coordinate-space stack beneath the fabric: the Tagma whitepaper defines coordinate identity, and the reference implementation lives at [docs.ssccs.org/projects/syntagma](https://docs.ssccs.org/projects/syntagma)
[^chton]: chton is the IO materialization layer: it lands the coordinate space onto physical origins (memory, file, signal, network) and provides the flat key-space IO surface that neXus backends implement; [docs.ssccs.org/projects/chton](https://docs.ssccs.org/projects/chton).
[^ssccs]: SSCCS whitepaper, Schema-Segment Composition Computing System, [ssccs.org/wp](https://ssccs.org/wp), DOI 10.5281/zenodo.18759106
[^tagma]: Tagma whitepaper, [docs.ssccs.org/projects/syntagma/tagma/](https://docs.ssccs.org/projects/syntagma/tagma/), DOI 10.5281/zenodo.21302508
[^coord20]: Coord ID 20 migration note, nexus repository, [github.com/ssccsorg/nexus/blob/main/docs/2026-08-20-coord-id-20-migration.md](https://github.com/ssccsorg/nexus/blob/main/docs/2026-08-20-coord-id-20-migration.md)
[^l2]: Content-hash conflict detection and the L2 restructure (2026-08-20) and The thin nexus principle (2026-08-20), ssccs-nexus repository docs; [github.com/ssccsorg/nexus/blob/main/docs/2026-08-20-176-content-hash-conflict-l2-restructure.md](https://github.com/ssccsorg/nexus/blob/main/docs/2026-08-20-176-content-hash-conflict-l2-restructure.md) and [github.com/ssccsorg/nexus/blob/main/docs/2026-08-20-thin-nexus-principle.md](https://github.com/ssccsorg/nexus/blob/main/docs/2026-08-20-thin-nexus-principle.md)
[^learn]: Computation as knowledge: deterministic path learning (2026-09-02), ssccs-nexus repository docs; [github.com/ssccsorg/nexus/blob/main/docs/2026-09-02-computation-as-knowledge-deterministic-path-learning.md](https://github.com/ssccsorg/nexus/blob/main/docs/2026-09-02-computation-as-knowledge-deterministic-path-learning.md)
[^bench]: neXus benchmark suite, [github.com/ssccsorg/nexus](https://github.com/ssccsorg/nexus), benches directory
