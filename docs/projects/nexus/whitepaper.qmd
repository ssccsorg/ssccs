...
This document is a technical record of the system as it exists in the reference implementation and in the accompanying documentation. Its intended uses are three: as a specification for implementers, as the archival basis for a registered DOI, and as a dated statement of the original ideas in the model for the purpose of prior-art defense. Sections 2 through 6 define the model. Sections 7 and 8 state verification results and measurements. Sections 9 through 13 delimit the ecosystem, its boundaries, related work, the roadmap, and the conclusion. Appendices carry the detailed specifications, protocol tables, verification catalogs, benchmark tables, and a glossary.
...
Lookup cost scales with coordinate depth, not with volume: ten thousand and $10^{24}$ records cost the same number of dereferences because the coordinate is the address.
...
Reverse lookup uses the inverse index from Facts to Intents, measured at 189 µs per 100 calls in this scenario. The earlier full-scan path cost 60.4 ms per call and is superseded by the inverse index.
