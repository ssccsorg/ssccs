  ...
  pdf:
    ...
    include-in-header:
      text: |
        ...
        \IfFileExists{./_files/nexus_whitepaper_metadata.tex}{\input{./_files/nexus_whitepaper_metadata.tex}}{\GenericWarning{}{Metadata: no generated file is present, so the version stamp and the affiliation values are empty. Run 'sdb pre .' to write them.}\providecommand{\affiliationname}{}\providecommand{\affiliationurl}{}\providecommand{\affiliationdomain}{}\providecommand{\authoremail}{}\providecommand{\authorrole}{}}
...
:::{#fig}
{{< include _include/bench/fig-bench-walk-peak.qmd >}}
:::
...
```{python}
#| label: fig-ecosystem
#| fig-cap: "One record model, many substrates. An instance is a configuration of the same model, and the record layer is unchanged from a native daemon to a hosted runtime."
dot("""
digraph Ecosystem {
    node [shape=box, style="rounded,filled", fontsize=10];
    edge [fontsize=8];

    Core [label="the record model\nFacts, Intents, Hints\none coordinate identity", shape=cylinder, fillcolor="#d8f0e0", penwidth=2];

    Skeleton [label="the conformance skeleton\nthe model in its smallest form", fillcolor="#e8ecf8"];
    Hosted [label="a hosted runtime\nworker, canister, edge", fillcolor="#e8ecf8"];
    Editor [label="a person's editor\nan agent as a peer", fillcolor="#e8ecf8"];
    Field [label="a field controller\nrobotics, sensing", fillcolor="#e8ecf8"];
    Verifier [label="a verification engine\nexhaustive enumeration", fillcolor="#e8ecf8"];

    Skeleton -> Core [dir=both];
    Hosted -> Core [dir=both];
    Editor -> Core [dir=both];
    Field -> Core [dir=both];
    Verifier -> Core [dir=both];
}
""")
```
