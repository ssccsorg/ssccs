...
| **TSSC** | **Topological Scheme-Segment Cluster**. A dynamic execution swarm of Bubbles where computation is governed by inter-bubble boundary dynamics (Membrane Computing). | $\text{TSSC} = \sum_{i} B_i \mid \partial B_i \cap \partial B_j \neq \emptyset$ <br/><br/> A collective computational swarm where multiple Bubbles interact through shared boundaries, enabling decentralized, high-scale execution. |
...
|------|------------|-------------------|
| O1.1 | State-space is the sole first-class entity. Data, time, and events are projections of state-space. | `∀x: x ≡ SchemeSegment` |
...
| R4.2 | All primitive types are equivalent one-dimensional projections of state-space. | `Type ≡ Projection(SchemeSegment¹)` |
