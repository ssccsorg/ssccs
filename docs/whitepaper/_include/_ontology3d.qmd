...
for i in range(1, num_segments+1):
    y = y_layer['scheme'] + random.uniform(-0.1, 0.1)
    all_coords[f's{i}'] = (random.uniform(0,12), y, random.uniform(-12,12))

for i in range(1, num_schemes+1):
    all_coords[f'sch{i}'] = (random.uniform(0,10), y_layer['scheme'], random.uniform(-5,5))

for i in range(1, num_fields+1):
    y = y_layer['field'] + random.uniform(-0.05, 0.05)
    all_coords[f'f{i}'] = (random.uniform(4,6), y, random.uniform(-5,5))
...
#| fig-cap: "A structural composition visualization."
...
plot_edges(scheme_seg_edges, 'lightgray', '-', lw=0.1)
plot_edges(field_influence_edges, 'gray', '--', lw=0.1)
plot_edges(scheme_to_field_edges, 'gray', '-', lw=0.1)
plot_edges(seg_to_field_edges, 'gray', '-', lw=0.1)
plot_edges(obs_to_field_edges, 'black', '-', lw=0.8)
...
trace_obs2f = create_edge_trace(obs_to_field_edges, color=BLACK, width=2, name='Observation→Field')
...
fig.update_layout(
    scene=dict(
        camera=dict(
            eye=dict(x=-1.577, y=-0.923, z=0.255),
