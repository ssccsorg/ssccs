...
digraph SSFormat_Topology {
    ...
    hw_note [label="Hardware Mapping"];
...
This example can be extended to illustrate sequential composition (first apply $F_{\text{similarity}}$, then $F_{\text{position}}$) or parallel composition (both Fields observe the same Segment simultaneously). The dot diagram above can be augmented to show those variants, but the core principle remains: Field composition merges constraints and transition matrices, producing a new Field that refines the observation semantics without altering the underlying Scheme.
