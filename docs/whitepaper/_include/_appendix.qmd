...
From these components emerge the key characteristics of a `.ss` specification:
...
This content‑based addressing ensures that the same topological blueprint always yields the same cryptographic hash, enabling verification and composition. A Schema’s identity is derived solely from its topological properties. Changing a physical implementation detail (e.g., cache‑line alignment) does **not** affect the `SchemeId`. However, altering the connectivity or the metric space produces a new, distinct identity:
