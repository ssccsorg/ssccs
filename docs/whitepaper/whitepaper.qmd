...
The primitives have been implemented as executable code—a cross-validated RISC-V simulation confirms deterministic results across independent substrates—and the same structural principle has been applied experimentally to existing open-spec silicon through ustom instruction verification via ExaVerif[@ev2026exaverif].

This reverse-validation on commercial silicon is not a target—it is a calibration. It demonstrates that the SSCCS model can be artificially instantiated within the constraints of existing architectures, serving as a proof of concept while the long-term trajectory remains native hardware where Schemes are instantiated directly rather than emulated. These empirical results ground the model in today's silicon, but they are not projections of SSCCS-native compute.

neXus[@nexus2026fih], the state-space engine developed alongside the reference implementation, archives every Observation as an immutable Fact—making the calibration process auditable, replayable, and composable across experiments. neXus is not part of the computational model; it is the archival implementation that preserves the model's empirical footprint.
