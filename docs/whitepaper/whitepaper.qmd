...
This description is more primitive than any hardware description language: it specifies only coordinate space, relations, and observation constraints. No registers, clocks, or circuits appear in `.ss` — because the same topology can compile to RTL, FPGA bitstream, CPU loop, or formal model without rewriting. The implementation is a target; the topology is the invariant.
