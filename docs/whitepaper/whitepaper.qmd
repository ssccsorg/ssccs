...
version-prefix: ssccs-doi.org/10.5281/zenodo.18759106
version-mark: true
  ...
  pdf:
    ...
    include-in-header:
      text: |
        ...
        \IfFileExists{./_files/whitepaper_metadata.tex}{\input{./_files/whitepaper_metadata.tex}}{\GenericWarning{}{Metadata: no generated file is present, so the version stamp and the affiliation values are empty. Run 'sdb pre .' to write them.}\providecommand{\affiliationname}{}\providecommand{\affiliationurl}{}\providecommand{\affiliationdomain}{}\providecommand{\authoremail}{}\providecommand{\authorrole}{}}
        ...
        \def\@maketitle{
          \begin{center}
            ...
            \vspace{2em}
                {\normalsize \@date \par}
                \vspace{0.3em}
                {\normalsize \texttt{\href{https://doi.org/10.5281/zenodo.18759106}{doi.org/10.5281/zenodo.18759106}} \par}
          \end{center}
          \vspace{6em}
        }
        \makeatother
...
title_meta_items = {
    "html": [
        {"title": "DOI", "link": "https://doi.org/10.5281/zenodo.18759106", "content": "10.5281/zenodo.18759106", "icon": "bi bi-file-text"},
        {"title": "View Slides", "link": "/works/pt.pdf", "content": "PDF", "icon": "bi bi-file-pdf"},
    ]
}
```
...
SSCCS (Schema–Segment Composition Computing System) replaces the sequential instruction‑centric model with a geometric, observation‑driven model. Computation is defined as the deterministic projection of immutable structural primitives under dynamic constraints. The model rests on four primitives: Segments (immutable coordinate points in a possibility space), Schemes (immutable blueprints that define structural relationships between segments), Fields (mutable constraint predicates that determine admissible segments), and Observation (the evaluation of a field against a scheme that produces a projection). Fields support composition through union, intersection, and product operations, and may be encrypted or cryptographically signed to enable secure composition across trust boundaries.
...
The model is architecture-agnostic: the same Scheme can target CPUs, FPGAs, or custom silicon without modification. A Rust-based proof of concept validates all core concepts, confirming that the abstract primitives produce deterministic results on existing silicon. An open format specification is under development [*-@sec-appendix-openformat*] and is updated as the specification evolves through implementations across projects.
...
Computation is observed synthesis, not execution over time. Observation activates the Field, collapses possibility, and produces a projection. The underlying structure remains unchanged. A projection, as the residue of collapse, is transient; it constitutes what we recognize as data or state. Yet throughout this entire process, the fundamental structure itself remains untouched and unaltered.
...
The primitives have been implemented as executable code, and deterministic behavior has been confirmed across four independent implementations of the same pipeline: the Rust fallback, hand-written RISC-V assembly under Spike, SystemVerilog, and the open-source ev (ExaVerif) verification CLI, pinned by golden anchors that are auto-verified on every validation run. An inverse-validation exercise on conventional open-spec silicon further instantiates the model within the constraints of an existing architecture (the calibration procedure and results are reported in Phase 1 of the roadmap [*-@sec-appendix-roadmap*]). Because Observation is deterministic by definition, every calibration is auditable and replayable by construction. These results ground the model in today's silicon; the long-term trajectory remains native hardware where Schemes are instantiated directly rather than emulated.
...
#| fig-alt: "Three-layer diagram showing static infrastructure (Segments + Scheme), dynamic substrate (Field), and the active event (Observation) producing a Projection"
dot("""
digraph SSCCS_Ontology {
    ...
    subgraph cluster_dynamic {
        label="Dynamic Substrate";

        node [shape=note];
        Field [label="Field F₁ = (C, T)\nconstraints + transitions"];
    }
    ...
    Scheme -> Field [label="binds"];
    s2 -> Field [label="anchors"];
    s3 -> Field;

    edge [arrowhead=normal, style=dashed];
    Observation -> Field [label="applies"];

    edge [arrowhead=normal];
    Field -> Projection [label="collapses"];

    edge [arrowhead=normal, style=dotted];
    Projection -> Data [label="interpret"];
}
""")
```
...
Its properties are: **Immutability** (once created, a Segment cannot be modified), **Statelessness** (contains no values, only coordinates and identity). Because Segments contain no mutable state, they can be observed concurrently by any number of observers without synchronization. The cryptographic identity ensures that every Segment is uniquely identifiable. For example, on the Unicode Basic Multilingual Plane (BMP), every code point is a fixed 16-bit value, and in a specific contiguous address range code points carry a closed-form compositional structure whose axes are recoverable by integer arithmetic alone. That combination of fixed width and structural composition is a general methodology for primitive computing: a coordinate serves directly as an address in OS-less environments, with no variable-length decoding, no parser, and no hash function. The boundary between the two identities is explicit: the hash identity applies wherever cryptographic integrity or content-derived identity is required, and the closed-form coordinate serves directly as identity where the coordinate structure is self-validating. The tagma block [@tagma2026wp] is an independent project that realizes this property as a concrete case (Appendix [*-@sec-appendix-tagma*]); the full identity analysis is developed in the Tagma whitepaper.
...
Formally, the five types contribute conjuncts to the Field predicate:
$$
c_{\text{dim}}(s) \equiv \bigwedge_{a} s.a \in I_a, \quad c_{\text{top}}(s) \equiv \text{dist}_R(s, s') \leq k, \quad c_{\text{alg}}(s) \equiv f(s.c) = 0,
$$
$$
c_{\text{phys}}(s) \equiv s.e \leq E_{\max} \land s.\tau \leq T_{\max}, \quad C(s) = \bigwedge_i c_i(s),
$$
where $\text{dist}_R$ is the shortest path length in the Scheme's relation graph, and logical constraints are realized through Field composition.
...
The observation operator is therefore a partial function over $\mathcal{S}$: it is defined only for segments in $\mathcal{A}(\Sigma, F)$. Outside the admissible set, no projection exists. Two segment sets are observationally equivalent under a Field when every observer yields the same projection for both: $S_1 \simeq_F S_2 \iff \forall \Omega.\, \Omega(S_1, F) = \Omega(S_2, F)$. The Field predicate reads a subset of the axes, so equivalence reduces to agreement on the axes it reads.
...
When a Field updates, any previously computed projection becomes stale; a subsequent observation reflects the new Field state. A failed update applies no partial change: the update is the identity on the Field, $\text{update}(F, t, c) = F$ on failure, the previous state is retained, and the failure is recorded on the Field. Partial application would break the determinism and auditability guarantees.

The determinism of an update depends on its trigger type. A temporal trigger is deterministic if its time source is monotonic and reproducible (a fixed-step clock or logical epoch counter); it is non-deterministic if derived from wall-clock time. A dependency trigger is deterministic if the update order is fixed, where the order is a topological order of the dependency graph and dependents observe against their committed versions during the cascade, and a provider updates a binding only after every dependent Field has re-observed the new version, so that $\forall F_d \in \text{Dep}(F_p): v_{\text{commit}}(F_d) \geq v(F_p)$ before the next provider update.
 An event trigger is deterministic only if the event source itself is deterministic. Observed and composite triggers inherit determinism from their constituents.
...
Observation results are ephemeral (see §3.4). Caching with versioning is an implementation optimisation—it does not change semantics. Each observation commits to the Field version it resolved against, so the returned result is consistent with that version: $\text{stale}(P) \iff v_{\text{commit}}(P) \neq v_{\text{current}}(F)$. Stale projections are transparently recomputed.
 The diagram below illustrates the dynamic observation cycle.
...
where $\times$ denotes independent composition of projections. The combined projection is therefore independent of the order of evaluation, up to observational equivalence, so the isolation equation extends to schedules. Every Segment and Scheme has a unique cryptographic hash, enabling identity-based boundaries where computations can only access Segments for which they hold valid references. This provides inherent structural isolation against interference without requiring additional security mechanisms.
...
Observation is deterministic: for identical $\Sigma$, $F$, and $t$, $\Omega$ always yields the same $P$. This follows from the definition of $\Omega$ as a function of $\Sigma$, $F$, and $t$; any non-determinism must be explicitly introduced through the resolution strategy in $O$. When the observation log is append-only, replaying the log from its initial state exactly reconstructs any prior projection without requiring snapshots or checkpoints. The log guarantee has a completeness half: whatever reaches an observer is reconstructable from the log, $\text{obs} \subseteq \text{replay}(L)$, because every input that determined a projection is in it.
 Field updates that are explicit transformations are likewise reversible through the log, which records every Field state; recovery restores the prior version by replay, $F_{t-1} = \text{replay}(L_{<t})$, and a failed update leaves the Field unchanged.
 The claim is conditional on a fixed Field: observed triggers are the one evolution path whose outcome depends on projection results, so a Field driven by observed triggers evolves along a feedback loop whose convergence is a separate safety condition (see [*-@sec-appendix-dynamic-field-evolution*]).
...
Concretely, the compiler constructs a directed graph where vertices are Segments and edges are relations of type *Dependency*. **Strongly connected components (SCCs)** of this dependency graph are identified; each SCC forms a candidate for sequential observation because dependencies within a component create cyclic constraints. Sub-graphs with no edges between them are marked as **independent** and can be scheduled in parallel. Formally, the dependency graph is $G = (V, E_D)$ with $E_D \subseteq R_{\text{dep}}$; independent subgraphs satisfy $E_D(G_1, G_2) = \emptyset$, and the observation order is a topological order of the SCC condensation $G/\text{SCC}$. For relations
 of type *Adjacency* and *Hierarchy*, the compiler uses them to guide locality optimization in later stages, but they do not impose observation ordering.
...
digraph SystemStack {
    ...
    subgraph cluster_runtime {
        ...
        ConstraintGate [label="Constraint Gate\n(C(s) evaluation)"];
        Projector [label="Projector\n(Ω)"];
        ObservationLog [label="Observation Log\n(append-only, replayable)", style=dashed];
    }
    ...
    Runtime -> ConstraintGate [label="evaluate C(s)"];
    ConstraintGate -> Projector [label="admissible set"];
    FieldVersion -> Projector [label="version check", style=dashed];
    Runtime -> ObservationLog [label="record observation", style=dashed];
    Projector -> ObservationLog [label="record projection", style=dashed];
    ...
    Compiler -> ConstraintGate [label="emit gates", style=dashed];
...
Upon receiving an `observe(scheme, field)` call, the runtime loads the pre-computed memory layout from the layout cache, evaluates the constraint gates of the Field, invokes the projector, records the observation in the append-only log, and returns the resulting projection. The projector translates the logical addresses produced by the compiler into hardware-specific memory accesses, using dedicated observation instructions when available. This decouples the structural mapping (done at compile time) from the dynamic observation (done at runtime), enabling portable performance across heterogeneous hardware.
...
Although SSCCS was developed without direct reference to prior work, its theoretical core reveals meaningful parallels with several established research domains and contemporary infrastructure trends.

### Dataflow Architectures

Dataflow models treat programs as directed graphs where nodes fire when inputs are available, eliminating the program counter and enabling data-driven execution. Both dataflow and SSCCS avoid global instruction sequencing. Dataflow execution is demand-driven by data availability. SSCCS computation is constraint-driven by Field observation. Dataflow relies on token movement between nodes. SSCCS keeps all Segments stationary and moves only the projection. Dataflow parallelism is bounded by token availability. SSCCS parallelism is bounded only by structural independence and physical propagation delay.

### Functional Programming

Functional languages enforce immutability and referential transparency, enabling equational reasoning and automatic parallelisation. This aligns with SSCCS immutable Segments and deterministic Observation. Both treat state as immutable, eliminating data races and simplifying verification. Referential transparency in functional programming corresponds to deterministic Observation in SSCCS. Functional languages execute on von Neumann hardware, inheriting the instruction-fetch and data-movement bottleneck. SSCCS is defined independently of the execution substrate. Its immutability is structural, not merely a language-enforced discipline. Functional programming operates over time-ordered sequences of operations. SSCCS treats time as one coordinate among many.

### Processing-in-Memory (PIM)

PIM architectures place compute units near or inside memory banks, reducing data movement between processor and memory. This directly addresses the von Neumann bottleneck that motivates SSCCS [*-@sec-appendix-integration-existing-toolchains*; @eyeriss2016; @ieee2023performancewalls; @ieee2025]. Both minimise data movement. PIM moves compute to data. SSCCS keeps data stationary and observes structure. PIM is a hardware optimisation within the von Neumann paradigm. It still executes instructions, still moves addresses, and still requires synchronisation. SSCCS eliminates the paradigm itself: no instruction fetch, no address decode, no data movement.

### RISC‑V Vector Extension (RVV)

RVV provides data-parallel processing through vector registers and SIMD-style instructions, exploiting regular data-level parallelism. Both exploit data parallelism and structured access patterns. RVV vectorised loops over contiguous memory regions resemble SSCCS structural adjacency mapped to memory layout. RVV still executes a sequential stream of vector instructions. SSCCS eliminates instruction sequencing entirely. Parallelism is implicit in the Scheme topology, not expressed as a vector-length-agnostic loop. RVV performance depends on register pressure and memory bandwidth. SSCCS performance depends on structural propagation delay. RVV is a programmer-visible feature. SSCCS is a substrate-visible abstraction.

### GraphBLAS

GraphBLAS defines graph algorithms in terms of linear algebra over semirings, enabling portable and optimised graph processing. Both treat computation as algebraic operations over structured data. GraphBLAS sparse matrix operations and SSCCS Field constraints over topological relations both express parallelism implicitly. GraphBLAS is an API and implementation standard targeting existing hardware. Its performance depends on optimised sparse kernels and memory layout. SSCCS defines computation as structural observation, not matrix algebra. GraphBLAS relies on efficient sparse storage formats like CSR and CSC that require index traversal. SSCCS eliminates indexing by using coordinates as addresses.

### Apache Arrow / Parquet

Arrow provides a standardised, language-agnostic columnar memory format for zero-copy data exchange. Parquet extends this to compressed, columnar storage for analytics. Both Arrow and Parquet are layout-first. Data representation is designed to match access patterns. Arrow columnar layout enables vectorised processing. SSCCS MemoryLayout abstraction similarly maps structural adjacency to physical address proximity. Arrow and Parquet are data-format optimisations within the existing compute stack. They still rely on index scans and query execution engines. SSCCS eliminates indexing altogether. Coordinate is address, so lookup is arithmetic, not a scan. Arrow reduces data movement between languages and processes. SSCCS eliminates data movement within a computation.

### CXL / Compute Express Link

CXL is an open standard interconnect for cache-coherent memory sharing between CPUs, accelerators, and memory devices. It enables memory pooling, tiering, and accelerator-attached memory. Both CXL and SSCCS recognise the need to unify memory and compute. CXL provides a physical and protocol layer for memory-centric computing. SSCCS defines a computational model where memory and compute are structurally unified. CXL is a hardware interconnect standard within the von Neumann ecosystem. SSCCS stationary Segments eliminate the need for address translation and memory referencing at the computational level.

### Safety and Accountability

Real-world incidents illustrate the need for deterministic reproducibility: autonomous vehicle collisions [@ntsb2019uber], financial flash crashes [@nanex2010flash; @sec2010flash], and diagnostic errors [@yang2025pathology] highlight the consequences of non-deterministic AI behavior. Recent analyses indicate that AI-related incidents have risen by more than 50% in the last year alone, with total damages exceeding $100 billion [@raiids2025]. SSCCS structural determinism offers a foundation for audit-by-design systems.

### Geometric Constraints and State-Space Models

Recent AI research demonstrates the growing relevance of structural constraints. Research such as *Manifold-Constrained Hyper-Connections* by DeepSeek [@deepseek2025manifold] highlights the efficacy of applying geometric inductive biases. This supports the SSCCS approach of defining computational processes through topological constraints. SSCCS also shares conceptual ground with State-Space Models (SSMs) like Mamba [@gu2023mamba]. While these systems achieve high-performance linear recurrences through ad-hoc kernel tuning, SSCCS redefines the recurrence not as a procedural loop, but as a one-dimensional Scheme of adjacent Segments.

### Concurrent Implementation Case: DeepSeek Spatiotemporal Composability

A concurrent implementation case. The DeepSeek paper on spatiotemporal composability [@shi2026spatiotemporal] (August 2026) formalizes dynamic software composition through revertible effects and reactive coeffects, six months after the SSCCS model of February 2026. It was developed independently. Its reference list contains no SSCCS work, and the parallel is convergent evolution from shared ancestors. Five structural patterns coincide with the model: explicit admissibility predicates, change classification, version-based staleness, algebraic composition, and commutation as the concurrency condition. The surface vocabulary carries different meanings, and the underlying definitions of computation differ. SSCCS defines computation as revelation. The case defines it as the mutation of a shared context with rollback machinery. The case is examined in the appendix [*-@sec-appendix-deepseek-comparison*].

### Recent Ecosystem Developments

Recent ecosystem developments in open-hardware space computing further contextualize SSCCS relevance. Initiatives such as ESA TRISTAN roadmap and radiation-tolerant RISC-V implementations (e.g., StarRISC, HARV-SoC) highlight the industry shift toward deterministic, mixed-criticality aware processors [@tristan2025]. SSCCS aligns with this trajectory by providing a software-defined resilience layer that complements physical hardening, enabling semantic fault detection, adaptive redundancy, and cryptographic policy enforcement on open silicon.

### Summary of Structural Differences

| Paradigm | Data Movement | Execution Model | Indexing Dependency | Verification |
|----------|---------------|-----------------|---------------------|--------------|
| Von Neumann (CPU/GPU) | Data moves to compute | Instruction stream, sequential | Hash tables, B‑trees | Runtime-dependent |
| Dataflow | Tokens move between nodes | Graph-driven, dataflow | Fixed graph, no indexing | Medium (static analysis) |
| Functional Programming | Data moves through transformations | Evaluated by reduction or graph rewriting | Language-dependent; inherits substrate indexing | High (referential transparency) |
| Processing-in-Memory | Compute moves near data; data still moves between banks | Instruction-based, near-memory | Inherits substrate indexing | Medium |
| RISC‑V Vector Extension | Data moves in vectorised loads/stores | Vectorised instruction stream | Inherits index-traversal cost | Medium |
| GraphBLAS | Sparse kernels over memory arrays | Matrix algebra over semirings | Inherits storage index traversal (CSR/CSC) | Medium (API-level) |
| Apache Arrow / Parquet | Zero-copy columnar access | Query-engine execution plans | Scan or index, depending on engine | Low (storage format only) |
| CXL | Cache-coherent memory sharing | Existing instruction-based model | Inherits substrate indexing | Medium |
| FPGA Overlays | Configuration bitstream loaded; data flows through fabric | Spatial, reconfigurable | Deterministic memory map | Medium (netlist analysable) |
| SSCCS | Segments stationary; only Projection transmitted | Structural Observation | **No indexing, coordinate is address** | High (deterministic by definition) |

These references contextualize SSCCS within the broader intellectual landscape. In each domain, the shift from execution to observation is expected to offer advantages that incremental optimization cannot provide. While SSCCS shares surface-level principles with many prior innovations, its integration of stationary data, coordinate-based addressing, and structural observation creates a distinct architectural category.
...
- Rust reference implementation organized as three paths: the host simulation workspace (17 crates, including 11 constitutional concept crates), the bare-metal RISC-V integration crate, and a standalone criterion benchmark suite.
- Deterministic reproducibility is confirmed across four independent implementations of the same pipeline: the Rust fallback, hand-written RISC-V assembly under Spike, SystemVerilog, and the ev (ExaVerif) verification CLI, pinned by 19 golden anchors that are auto-verified on every validation run. The inverse-validation exercise described in the Introduction further demonstrates the model's applicability to conventional RISC-V silicon [@ev2026exaverif].
- The benchmark suite covers three kernels (vector addition, 2D convolution, graph BFS), each expressed as a Rust baseline and an SSCCS formulation; it records emulation overhead on existing hardware as validation evidence. Ongoing work measures determinism, implicit parallelism, and data movement reduction on larger benchmarks.
...
The SSCCS Proof of Concept (PoC) is the Rust-based reference implementation of the core ontological layers defined in this specification. It corresponds to Phase 1 (software emulation) of the development roadmap and is organized as three paths: the standard host simulation workspace, the bare-metal RISC-V integration crate, and a standalone benchmark suite. This appendix documents implementation details that complement the open-format specification [*-@sec-appendix-openformat*].
...
The same constraint pipeline is implemented independently on three substrates and cross-verified automatically:

- **Rust fallback:** the reference path on the host (x86_64/aarch64), exercised by the workspace tests.
- **RISC-V assembly:** five hand-written modules covering observation, collapse, field update, scheme layout, and adjacency, executed at the ISA level under Spike.
- **SystemVerilog:** 18 modules implementing constraints, projectors, composition, and the CORE-V XIF coprocessor stub.

Nineteen golden anchors pin the assembly outputs; a Python checker forces the SystemVerilog constants and the Rust fallback to agree with them on every validation run. The open-source ev (ExaVerif) CLI reproduces the anchor results through its own constraint engine, forming a fourth independent channel [@ev2026exaverif].

The workspace also emits the Scheme as an assembly `.rodata` section (`SCHEME_SEG_COUNT`, `SCHEME_COORDS`, `SCHEME_ADDRESSES`, `GOLDEN_*`) and generates branchless constraint gates with zero conditional branches. The bare-metal crate consumes both through `observe_scheme`, a batch observation over the emitted tables, completing the handoff from the reference simulation to the practical plane.
...
A signed Field binary replaces an active Field in place through a defined lifecycle, $\text{replace}(F_{\text{old}}, F_{\text{new}}) = \text{verify}(F_{\text{new}}) \to \text{swap} \to \text{rollback}(F_{\text{old}})$, where rollback uses the log-based recovery of the previous version.
...
### Observation Rules (Draft) {#sec-appendix-observation-rules}
...
Determinism of a composite trigger is the logical combination of determinism of its parts (AND/OR preserve determinism if all parts are deterministic; sequencing preserves determinism if the sequence is prescribed). Formally, each trigger type is an update rule:
$$
F' = \text{evolve}(F, \Delta t) \quad \text{(temporal)}, \quad F' = \text{update}(F, e, \text{ctx}) \quad \text{(event)},
$$
$$
F' = \text{update}(F, P \models \psi) \quad \text{(observed)}, \quad F' = \text{update}(F, v(F_{\text{other}})) \quad \text{(dependency)},
$$
with composite triggers combining the guards, $t = t_1 \land t_2 \lor t_3$.
...
Each Field maintains a monotonically increasing version number. Initially `version = 0`. On every update (triggered evolution), `version += 1`. Formally, $v(F) \in \mathbb{N}$ with $v_0(F) = 0$ and $v \mapsto v + 1$ on every update; a cached projection is valid iff $v_{\text{cache}} = v_{\text{current}}(F)$.
...
fn observe(scheme: &Scheme, field: &Field, field_version: u64) -> Projection {
    ...
    CACHE.insert(
        (scheme.id(), field.id()), CachedEntry { value, version: field_version }
    );
    value
}
```
...
| ExaVerif [@ev2026exaverif] | Fields remain stationary; only constraint evaluation crosses the verification boundary | Implicit (Cartesian enumeration across field domains) | High (exhaustive: 33.5M/33.5M evaluated) |
...
## Tagma: Coordinate Identity and Its Measurement {#sec-appendix-tagma}
...
{{< include ../projects/syntagma/_include/bench/fig-bench-identity.qmd >}}

### Addressable space and spatial query

The addressable space grows as $11{,}172^N$, and lookup cost does not follow the space. The dense path holds at 0.39 ns at every depth; the tree fallback costs O(N) dereferences bounded by schema depth, independent of data volume. Coordinate queries are spatial computation: a compound axis filter resolves as bitwise AND over the coordinate set at 329 Melem/s, while a hash table scan of the same data sustains 2.4 Melem/s.[^tagma-bench]

:::{ layout-ncol=2 }

{{< include ../projects/syntagma/_include/bench/fig-bench-address-space.qmd >}}

{{< include ../projects/syntagma/_include/bench/fig-bench-spatial.qmd >}}

:::

[^tagma-bench]: The figures in this section are single-sourced: the same include files render in the [Tagma whitepaper](https://docs.ssccs.org/projects/syntagma/tagma/), [Tagma Identification](https://docs.ssccs.org/projects/syntagma/tagma/identification/), [TagmaMap](https://docs.ssccs.org/projects/syntagma/tagma/map/), and the [synTagma project brief](https://docs.ssccs.org/projects/syntagma), so this appendix carries the current measurements without duplication. The full benchmark suite (51 microbenchmarks across 12 criterion groups, including bulk, stress, deep-tree, resource-efficiency, and Map groups) and the reference implementation are documented in the [Tagma whitepaper benchmark appendix](https://docs.ssccs.org/projects/syntagma/tagma/#sec-appendix-benches).


### Coordinate Identity in Key-Value Storage {#sec-appendix-tagma-map}

TagmaMap exposes a HashMap-compatible API (insert, get, remove, contains_key) backed by the coordinate space, replacing the hash layer with a closed-form arithmetic mapping: zero collision probability, no bucket probing, no resizing. On ARMv8.4-A Firestorm silicon, string-key conversion costs 5 ns, roughly three times faster than SipHash-2-4, and CoordMap2 get latency is scale-invariant (22.0 ns at 10k, 21.4 ns at 1M, 22.1 ns at 10M) while HashMap rises with memory pressure; the native dense path reaches 0.39 ns, and spatial queries (prefix scans, axis projections, range queries) resolve as bitwise operations at 94 ns. One prominent case is the transformer KV cache, where lookup dominates memory traffic as context grows. Documentation: [TagmaMap](https://docs.ssccs.org/projects/syntagma/tagma/map/).

Realization insight. Because the coordinate is the address, the index layer disappears and lookup cost stops following data volume; the measured scale invariance is the empirical form of the structural-isolation claim.

:::{ layout-ncol=2 }

{{< include ../projects/syntagma/_include/bench/fig-bench-map-micro.qmd >}}

{{< include ../projects/syntagma/_include/bench/fig-bench-map-scale.qmd >}}

:::

### Hardware Realization of the Primitive {#sec-appendix-tagma-hardware}

Tagma scope: one primitive definition, two realization substrates. The same composition formula evaluates in arithmetic in software and in logic gates in hardware, and the two must agree on every input. The hardware series measures the decoder on an open toolchain (Yosys, nextpnr, icetime): 478 cells in the ev-compatible generic schema and 588 cells at the 2-input gate level, with 201 LUT4 plus 41 carry cells on iCE40, closing the 12 MHz board clock at 16.79 MHz Fmax. The roughly 300-gate claim of the Tagma whitepaper holds for the naive shift-subtract decoder (206-232 cells); timing closure requires a multiply-shift structure at roughly 2.5x the gates. The hardware value is energy, determinism, and physical embedding: the N=1 dense case already fits in L1 cache, which software exploits at 0.38 ns per access. Every artifact is verified exhaustively over all 11,172 valid syllables in three independent channels (formula, Rust reference, gate netlist), with formal equivalence between RTL and the synthesized netlist over all 2^16 inputs. Documentation: [Hardware](https://docs.ssccs.org/projects/syntagma/tagma/hardware/).

Realization insight. The scope is itself a realization of the model: the composition formula is the Segment address space made executable, and the substrate split shows that the primitive's identity is definitional, not material. The measured gate count bounds the physical footprint, and the three-channel exhaustive verification carries the cross-validation principle of the main text into silicon.

:::{ layout-ncol=2 }

{{< include ../projects/syntagma/_include/bench/fig-hw-gates.qmd >}}

{{< include ../projects/syntagma/_include/bench/fig-hw-fmax.qmd >}}
...
## A Recent Implementation Case: DeepSeek Spatiotemporal Composability {#sec-appendix-deepseek-comparison}

The DeepSeek paradigm [@shi2026spatiotemporal] (August 2026) is one recent concrete implementation case of dynamic composition on von Neumann hardware, comparable to the ecosystem projects rather than to the model itself. It was developed independently: its reference list contains no SSCCS work, and the parallel is convergent evolution from shared ancestors. Five structural patterns coincide with the model: explicit admissibility predicates, change classification, version-based staleness, algebraic composition, and commutation as the concurrency condition. The surface vocabulary (observation, projection, constraint, composition) carries different meanings, and the underlying definitions of computation differ: SSCCS defines computation as revelation, while the case defines it as the mutation of a shared context with rollback machinery.

The case sharpened nine model-level definitions, each audited against the five foundational principles and the structural isolation requirement. Three are genuinely new definitions (observational equivalence, withdrawal ordering, failure semantics); the remaining six (committed version, observation transposition, reversible updates, the session-log invariant, binary Field replacement, declarative determinacy) are precisions of content the whitepaper already carried. Elements that import a globally mutable context, a realm-based resolution layer, or an orchestrator were excluded. The detailed audit and the engineering program are documented in the project notes.

| Definition | Canonical location |
|---|---|
| Observational equivalence | Observation and Projection |
| Committed version | Field Evolution and Observation Transition |
| Observation transposition | Structural Isolation |
| Withdrawal ordering | Field Evolution and Observation Transition |
| Reversible updates | Determinism and Auditability |
| Session-log invariant | Determinism and Auditability |
| Binary Field replacement | Fault Tolerance Computing in Extreme Environments |
| Failure semantics | Field Evolution and Observation Transition |
| Declarative determinacy | Field Composition |
