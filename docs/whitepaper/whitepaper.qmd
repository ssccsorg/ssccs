...
title_meta_items = {
    "html": [
        {"title": "View Slides", "link": "/proposal/pt.pdf", "content": "PDF", "icon": "bi bi-file-pdf"},
    ]
}
```
...
Recent work on classical parallelism provides convergent evidence. Erementchouk and Mazumder [@erementchouk2026symmetry] prove that relaxed spin networks with continuous symmetry perform multiple simultaneous computations under isotonicity constraints, while Kish [@kish2025exponential] demonstrates that classical noise-based logic achieves exponential speedups for certain search problems. These results independently reinforce the viability of deterministic parallel computation without quantum resources. A detailed comparison with SSCCS primitives is provided in [*-@sec-appendix-classical-parallelism*].
...
For systems that require strict reproducible execution, only deterministic triggers (e.g., fixed‑rate temporal triggers or dependency chains) are permitted. Non‑deterministic triggers (external events, observed predicates that depend on runtime values) are allowed only when reproducibility is not required.
...
where $E_{\text{obs}}$ is the energy required to perform a single observation, $E_{\text{update}}$ is the energy required to mutate the Field, $N_{\text{obs}}$ is the number of observations, and $N_{\text{update}}$ is the number of field updates. There is no term for moving data between memory and processor, because Segments are stationary. This model predicts that energy consumption scales with the number of observations and field updates, but not with data movement, which is a key source of energy inefficiency in traditional architectures [@horowitz2014computing]. Recent forecasts indicate that data center power consumption could double within four years [@gartner2026; @epri2026], with AI workloads driving much of this growth [@omdia2025; @mrl2025].
...
This determinism is concretely realized when the observation log is append-only. Because each observation event is recorded as an immutable entry, replaying the log from its initial state exactly reconstructs any prior projection without requiring auxiliary mechanisms such as snapshots, checkpoints, or incremental state summaries. The append-only invariant guarantees that the sequence of observations is total and irreversible, which is precisely the condition under which $\Omega(\Sigma, F)$ remains a pure function across repeated evaluations. No additional synchronisation or reconciliation layer is needed to preserve deterministic reconstruction.
...
- **Open RISC-V Synergy**: Concrete validation pathways are being explored via the RISC‑V ecosystem [@riscv2019spec], particularly through OpenHW CORE‑V’s eXtension Interface (XIF) [@openhw2025xif]. Custom observation primitives (`OBSERVE`, `COLLAPSE`) can be implemented as tightly coupled coprocessor extensions, enabling deterministic structural projection with minimal pipeline interference and formal verifiability.
- **OpenHW Ecosystem**: The OpenHW CORE‑V MCU [@openhw2025cvmcu] and SDK provide ready‑to‑use platforms for prototyping SSCCS observation units, while the Imperas simulation environment enables pre‑silicon verification. The CVA6 core and other OpenHW MCU variants offer scalable performance targets. Integration proposals outline concrete steps for mapping SSCCS primitives to RISC‑V extensions.
- **Edge and Peripheral Integration**: Peripheral integration with eFPGA fabrics [@quicklogic2024] and always‑on vision sensors demonstrates the versatility of the RISC‑V ecosystem for edge deployment, while formal verification tools [@smith2025] and coprocessor examples [@xifcopro2025] ensure correctness and extensibility. Efficient I/O for large‑scale machine learning [@emlio2025] further highlights the importance of minimizing data movement.
- **Space‑Grade Resilience**: Radiation‑hardened RISC‑V implementations such as HARV‑SoC and StarRISC, together with ESA’s TRISTAN roadmap [@tristan2025], demonstrate the industry shift toward deterministic, mixed‑criticality aware processors. Recent advances in multi‑bit fault‑tolerant instruction decoding [@ghosh2025], dynamic lockstep redundancy [@simoes2024], and high‑performance space computing platforms [@microchip2025pic64; @starlab2026] further illustrate the RISC‑V community’s focus on resilience. SSCCS complements these efforts by providing a software‑defined resilience layer that can be deployed as cryptographically signed Fields. [@elash2025efficacy; @walsemann2023radiation].

Taken together, these hardware directions point toward a deeper departure from conventional computing than PIM alone achieves. PIM reduces data-movement distance by placing compute near memory. SSCCS eliminates data movement as a concept: data remains stationary at its scheme coordinate, and computation consists only of constraint resolution across the topology. Native SSCCS hardware would replace the instruction pipeline with a constraint-resolution fabric.
